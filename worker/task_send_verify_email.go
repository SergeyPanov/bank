...
func (d *RedisTaskDistributor) DistributeTaskSendVerifyEmail(ctx context.Context, payload *PayloadSendVerifyEmail, opts ...asynq.Option) error {
	...
	if err != nil {
		return fmt.Errorf("failed marshaling the payload: %w", err)
	}
...
func (p *RedisTaskProcessor) ProcessTaskSendVerifyEmail(ctx context.Context, task *asynq.Task) error {
	...
	url := fmt.Sprintf("http://localhost:8080/v1/verify_email?email_id=%d&secret_code=%s", verifyEmail.ID, verifyEmail.SecretCode)
	content := fmt.Sprintf(`Hello %s,<br/>Click here to verify your email <a href="%s">link</a>`, user.FullName, url)
	to := []string{user.Email}

	err = p.mailer.SendEmail("Welcome to the SImple Bank", content, to, nil, nil, nil)
	if err != nil {
